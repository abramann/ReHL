...
BOOL WINAPI DllMain(HMODULE, DWORD r, LPVOID)
{
	...
	{
		VersionDllInit();
		if (MessageBoxA(NULL, "Setup hooks", "ReHL Loader", MB_OKCANCEL) != 2)
		{
			auto loaded = LoadLibraryA("rehl.dll");
			if(!loaded)
				MessageBoxA(NULL, "Failed to load rehl.dll. Game will run normally.", "ReHL Loader", MB_OK);
		}
	}
	return TRUE;
}
