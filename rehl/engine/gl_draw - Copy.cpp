...
ARRAY(unsigned int, trans_25102, [307202], 0x3E7E3);
ARRAY(unsigned int, scaled_25071, [524288], 0x3E51D);
...
{
	//return Call_Function<void, unsigned int*, int, int, qboolean, int, int>(0x3E320, data, width, height, mipmap, iType, filter);
	int w;
	int h;
	int bpp;
	...
	else
	{
		...
		case GLT_STUDIO:
			for (int j = 0; j < count; ++j)
				trans_25102[j] = (*(unsigned int*)&pPal[765]) & 0xFFFFFF | (data[j] << 24);
		...
		case GLT_WORLD:
			for (int k = 0; k < count; ++k)
				trans_25102[k] = (data[k] << 24) | (*(unsigned int*)&pPal[3 * data[k]]) & 0xFFFFFF;
			//for (int k = 0; k != count; ++k)
			//	trans_25102[k] = (data[k] << 24) | *(DWORD *)&pPal[3 * data[k]] & 0xFFFFFF;
			...
			if (count > 0)
			{
				...
				for (int l = 0; l != count; ++l)
				{
					while (1)
					{
						...
						if (v11 == 255)
							break;
						trans_25102[l++] = *(DWORD *)&pPal[3 * v11] | 0xFF000000;
						if (l == count)
						{
							iType = GLT_SYSTEM;
							GL_Upload32(trans_25102, width, height, mipmap, iType, filter);
				...
				do
				{
					uchar* v12 = &pPal[2 * data[k] + data[k]];
					char v13 = v12[2];
					char v14 = *v12 | (*v12 >> 6);
					char v15 = v12[1];
					*v12 = v14;
					...
					v12[1] = v16;
					v12[2] = v17;
					trans_25102[k++] = v14 | ((v16 | ((*(DWORD *)&v17 | 0xFFFFFF00) << 8)) << 8);
				} while ((int)k < count);
				*/
				...
				/*		bool systemType = true;
						for (int l = 0; l < count; ++l)
						{
							while (1)
							{
								...
								trans_25102[l++] = (*(unsigned int*)&pPal[3 * data[l]]) | 0xFF000000;
								if (l == count)
									break;
							}
