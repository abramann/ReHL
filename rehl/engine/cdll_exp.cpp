...
void hudPlaySoundByName(char* szSound, float volume)
{
	...
	if (pSound)
		S_StartDynamicSound(g_pcl.viewentity, CHAN_ITEM, pSound, r_origin, volume, 1.0, 0, PITCH_NORM);
	else
...
void hudPlaySoundByIndex(int iSound, float volume)
{
	...
	if (iSound >= MAX_SOUNDS || iSound < 0)
	{
		Con_DPrintf("invalid sound %i\n", iSound);
		return;
	}

	S_StartDynamicSound(g_pcl.viewentity, CHAN_ITEM, g_pcl.sound_precache[iSound], r_origin, volume, 1.0, 0, PITCH_NORM);
}
...
void hudPlaySoundVoiceByName(char* szSound, float volume, int pitch)
{
	...
	if (pSound)
	{
		...
		S_StartDynamicSound(g_pcl.viewentity, CHAN_BOT, pSound, r_origin, volume, 1.0, 0, pitch);
	}
	else
	{
		Con_DPrintf("invalid sound %s\n", szSound);
...
void hudPlaySoundByNameAtPitch(char* szSound, float volume, int pitch)
{
	...
	if (pSound)
	{
		...
		S_StartDynamicSound(g_pcl.viewentity, CHAN_ITEM, pSound, r_origin, volume, 1.0, 0, pitch);
	}
	else
	{
		Con_DPrintf("invalid sound %s\n", szSound);
