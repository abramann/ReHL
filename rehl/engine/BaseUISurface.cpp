...
static vgui::Dar<vgui2::VPANEL> staticPopupList;
...
void BaseUISurface::DrawSetTextureRGBAWithAlphaChannel(int id, const unsigned char* rgba, int wide, int tall, int hardwareFilter)
{
...
{
	cl_enginefuncs.pfnPlaySoundByName(const_cast<char*>(fileName), 1.0);
}
...
void BaseUISurface::PaintTraverse(vgui2::VPANEL panel)
{
	...
	vgui2::ipanel()->PaintTraverse(targetPanel, true, true);

	for (int i = 0; i < staticPopupList.Count(); ++i)
	{
		if (vgui2::ipanel()->Render_GetPopupVisible(staticPopupList[i]))
		{
			vgui2::ipanel()->PaintTraverse(staticPopupList[i], true, true);
		}
	}
}
...
void BaseUISurface::SetAsTopMost_( bool state )
{
	//Nothing
}

void BaseUISurface::SetAsToolBar_( bool state )
{
	//Nothing
}
...
void BaseUISurface::EnableMouseCapture_( bool state )
{
	//Nothing
}
...
void BaseUISurface::SetNotifyIcon_( vgui2::Image *, vgui2::VPANEL, char const* )
{
	//Nothing
}
...
CFontTextureCache::CFontTextureCache()
	: m_CharCache(0, 256, &CFontTextureCache::CacheEntryLessFunc), m_PageList()
{
